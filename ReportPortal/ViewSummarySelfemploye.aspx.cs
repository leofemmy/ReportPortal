...
{
    public partial class ViewSummarySelfemploye : System.Web.UI.Page
    {
        ...
        void calReport()
        {
            ...
            else if (sessions.MerchantCode.ToString() == "OGSS")
            {
            ...
            string strquery = String.Format("SELECT RevenueOfficeID, RevenueOfficeName, COUNT(*) AS Rec_Count FROM dbo.vwPayerInfo WHERE RevenueOfficeName IS NOT NULL AND RegTypeCode = 'DA' AND DateCreated BETWEEN '{0}' AND '{1}' AND MerchantCode ='{2}' GROUP BY RevenueOfficeID, RevenueOfficeName HAVING COUNT(*) > 0 ORDER BY RevenueOfficeName ASC", startdate, enddate, sessions.MerchantCode.ToString());
        ...
        public static void loadchildreport(string stoffice)
        {
            ViewSummarySelfemploye vp = new ViewSummarySelfemploye();

            var vartaxyear = stoffice;

            vp.callReport2(vartaxyear);


        }
        void callReport2(string stroffices)
        {
            Session["TaxOfficeName"] = stroffices;
        }
    }
}
