...
{
    public partial class ViewLineSummaryTaxOffice : System.Web.UI.Page
    {
        ...
        void calReport()
        {
            ...
            string strquery = String.Format("SELECT PayerUtin,PayerName,RevenueOfficeID,RevenueOfficeName,COALESCE(SUM(TaxPayable), 0) Assessment,COALESCE(SUM(PaymentAmount), 0) Payment,COALESCE(SUM(TaxPayable), 0) - COALESCE(SUM(PaymentAmount), 0) Differences FROM dbo.ViewPayerLedger WHERE AssessmentYear BETWEEN {0} AND {1} GROUP BY PayerUtin,PayerName, RevenueOfficeID,RevenueOfficeName ORDER BY PayerName ASC", varyear, varto);
        ...
        {
            ViewLineSummaryTaxOffice vp = new ViewLineSummaryTaxOffice();
